  ...
  return (
    <Reveal delay={0.05 * index}>
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        style={{
          padding: "24px", background: "rgba(255,255,255,0.03)", border: `1px solid ${hovered ? color : "rgba(255,255,255,0.1)"}`,
          transition: "all 0.3s", height: "100%", backdropFilter: "blur(10px)", position: "relative", overflow: "hidden"
        }}>
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: "#000", color: "#fff", minHeight: "100vh", width: "100%", overflowX: "hidden" }}>
      ...
      {/* HEADER */}
      <header style={{ position: "fixed", top: 0, width: "100%", zIndex: 100, padding: "15px 5%", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(0,0,0,0.8)", backdropFilter: "blur(10px)" }}>
        <img src={aurenLogo} alt="Logo" style={{ height: "30px" }} />
        <a href={WHATSAPP_LINK} style={{ textDecoration: "none" }}>
          <button style={{ border: "1px solid #00f0ff", background: "transparent", color: "#00f0ff", padding: "8px 16px", fontSize: "0.7rem", fontFamily: "'Orbitron'" }}>CONTATO</button>
        </a>
      </header>

      {/* HERO */}
      <section style={{ padding: "120px 5% 60px", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <h1 style={{ fontSize: "clamp(1.8rem, 8vw, 4rem)", fontFamily: "'Orbitron'", fontWeight: 900 }}>A MÁQUINA DE <br/> 
            <span style={{ color: "#00f0ff", textShadow: "0 0 20px #00f0ff" }}>{words[wordIndex]}</span>
          </h1>import { useEffect, useRef, useState } from "react";
import { MessageCircle, ArrowRight, Target, Bot, Crosshair, Layout, CalendarCheck, RotateCcw, BarChart3, Zap, ChevronDown, TrendingUp, Users, Clock } from "lucide-react";
import authorPhoto from "@/assets/image_9.png";
import aurenLogo from "@/assets/image_6.png";
...
          <span style={{ color: "rgba(255,255,255,0.2)", fontFamily: "'Orbitron', sans-serif", fontSize: "clamp(0.48rem, 1vw
          <p style={{ margin: "20px auto", maxWidth: "600px", color: "rgba(255,255,255,0.6)", fontSize: "1rem" }}>Atraímos clientes qualificados e automatizamos seu atendimento com IA.</p>
          <div style={{ display: "flex", gap: "15px", justifyContent: "center", flexWrap: "wrap" }}>
            <a href={WHATSAPP_LINK} style={{ background: "#00f0ff", color: "#000", padding: "15px 30px", fontWeight: "bold", textDecoration: "none", borderRadius: "2px" }}>QUERO CLIENTES</a>
          </div>
        </div>
      </section>

      {/* STATS */}
      <section style={{ padding: "60px 5%", background: "#050505", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "30px", textAlign: "center" }}>
        <div><TrendingUp color="#00f0ff"/><h2 style={{ fontSize: "2rem", color: "#00f0ff" }}><Counter target={150} suffix="+" /></h2><p>Agendamentos</p></div>
        <div><Users color="#ff00ff"/><h2 style={{ fontSize: "2rem", color: "#ff00ff" }}><Counter target={15} suffix="/dia" /></h2><p>Novos Leads</p></div>
        <div><Clock color="#7fff00"/><h2 style={{ fontSize: "2rem", color: "#7fff00" }}><Counter target={24} suffix="h" /></h2><p>IA Ativa</p></div>
      </section>

      {/* SERVICES */}
      <section id="services" style={{ padding: "80px 5%" }}>
        <h2 style={{ textAlign: "center", marginBottom: "40px", fontFamily: "'Orbitron'" }}>NOSSAS SOLUÇÕES</h2>
